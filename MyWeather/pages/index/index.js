...
Page({
  ...
  updateTopCity: function (event) {
    ...
    var current = event.detail.current % 3;
    try { topCity.left = weatherData[citySelected[(current + 2) % 3]].realtime.city_name; } catch (e) { }
    try { topCity.center = weatherData[citySelected[current]].realtime.city_name; } catch (e) { }
    try { topCity.right = weatherData[citySelected[(current + 1) % 3]].realtime.city_name; } catch (e) { }
  ...
  setHomeData: function (citySelected, weatherData) {
    ...
    try { topCity.left = weatherData[citySelected[2]].realtime.city_name; } catch (e) { }
